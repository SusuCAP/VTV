...
continuity_snapshots = Table(
    ...
    Column("project_id", PGUUID(as_uuid=True), ForeignKey("projects.id", ondelete="CASCADE"), nullable=False),
    Column("episode_id", PGUUID(as_uuid=True), ForeignKey("episodes.id", ondelete="CASCADE"), nullable=False),
    Column("shot_id", PGUUID(as_uuid=True), ForeignKey("shots.id", ondelete="CASCADE"), nullable=False),
    ...
    Column("localization_release_id", PGUUID(as_uuid=True), ForeignKey("localization_releases.id", ondelete="SET NULL")),
...
cost_events = Table(
    ...
    Column("model_release_id", PGUUID(as_uuid=True), ForeignKey("model_releases.id", ondelete="SET NULL")),
...
runtime_profiles = Table(
    ...
    Column("supported_operators", JSONB, nullable=False, server_default="[]"),
    Column("self_test_status", String(32), nullable=False, server_default="PENDING"),
    Column(
    ...
    Column("oom_test_status", String(32), nullable=False, server_default="PENDING"),
    Column("rollback_verified", Boolean, nullable=False, server_default="false"),
    Column("validation_evidence", JSONB, nullable=False, server_default="{}"),
    ...
    UniqueConstraint(
        "profile_name",
        "profile_version",
        name="uq_runtime_profiles_name_version",
    ),
    UniqueConstraint(
        "image_digest",
        "profile_version",
        name="uq_runtime_profiles_image_version",
    ),
)
...
review_tasks = Table(
    ...
    Column("assignee_id", PGUUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL")),
    Column("shot_id", PGUUID(as_uuid=True), ForeignKey("shots.id", ondelete="CASCADE")),
    Column("episode_id", PGUUID(as_uuid=True), ForeignKey("episodes.id", ondelete="CASCADE")),
...
provenance_manifests = Table(
    ...
    Column("id", PGUUID(as_uuid=True), primary_key=True),
    Column("project_id", PGUUID(as_uuid=True), ForeignKey("projects.id"), nullable=False),
    Column("delivery_id", PGUUID(as_uuid=True), ForeignKey("deliveries.id", ondelete="SET NULL")),
    Column("episode_id", PGUUID(as_uuid=True), ForeignKey("episodes.id", ondelete="SET NULL")),
...
model_access_profiles = Table(
    ...
    Column("profile_version", Integer, nullable=False, server_default="1"),
    Column("access_kind", String(32), nullable=False, server_default="UNKNOWN"),
    ...
    Column("launch_command", Text),
    Column("provider_model_id", String(256)),
    Column("provider_lifecycle", String(32)),
    ...
    UniqueConstraint(
        "model_release_id",
        "profile_version",
        name="uq_model_access_profiles_release_version",
    ),
)
