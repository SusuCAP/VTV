...
`INGEST_VALIDATE → PROXY_GENERATE → {SHOT_DETECT, AUDIO_STEM_SEPARATION}`，随后 dialogue stem
进入 `ASR_ALIGN`，代理与镜头清单进入 `VISION_ANALYSIS`。

其中视觉分析同时依赖代理和镜头结果。所有集的 `ASR_ALIGN` 与 `VISION_ANALYSIS` 最终汇聚
到唯一 `PROJECT_SYNTHESIS`。N 集任务总阶段数为 `6N + 1`；没有已上传 Episode 时 API
