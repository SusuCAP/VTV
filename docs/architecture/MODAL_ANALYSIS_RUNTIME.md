...
镜像固定 Python 3.12，安装 FFmpeg 和锁定版本的 Python 依赖，并仅复制运行所需 workspace
源码。分析函数限制 2 CPU、4 GiB 内存、1 小时超时和两次平台重试。业务幂等仍由
Stage Attempt/数据库条件提交保证，Modal 调用 ID 不作为业务 ID。

本地编排器新增 `--worker-mode modal`。媒体阶段仍在本地路由，三个分析阶段通过
