...
生产调度优先读取 Registry 已准入 release 注入的 `model_runtime` 参数，并用
`stage_runs.model_release_id` 固化选择结果；bearer token 仍只从对应环境变量读取。没有
Registry 注入时，可用 `VTV_AUDIO_ANALYSIS_*`、`VTV_VISION_ANALYSIS_*` 配置显式远程模式，
模式由 `VTV_ANALYSIS_ADAPTER_MODE=remote` 开启。
